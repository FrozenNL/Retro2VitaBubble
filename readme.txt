...
o  Add support for multifile systems (like DOSBOX), support is there already but not to pack them inside the VPK, you
   just need the starter file and add it to the _IN folder on the same location as on your PS Vita. The new system will
   allow you to create folders inside a MULTI folder, which does need a SETUPR2B.conf file which specify the startup
   file and eventual some other parameters.

o  Hoping there will also be Amiga (uae4all2) support, there needs to be a starter for it in order to create this, I am
   willing to build support for it once the starter eboot is available and once the aue4all2 supports to open files
   from a parameter.

Any questions, feature requests or found any bugs? Please email me at jordy-at-polfliet[d-o-t]net I try to respond ASAP.
